...
import ctypes
from ctypes import *
from marvin_smach.msg import Command
...
def my_callback(request):
    ...
    try:
        if sys.platform.startswith('win32'):
            ...
            if platform.machine().startswith('arm'):
                ...
                libPath = srcDir + "/../catkin_ws/src/prj-iki-robotics/ros-bci/marvins_bci/bin/linux64/libedk.so"
            libEDK = CDLL(libPath)
    ...
    def logEmoState(userID, eState):
        print IS_GetTimeFromStart(eState)
        print userID.value
        print IS_GetWirelessSignalStatus(eState)
        print IS_FacialExpressionIsBlink(eState)
        print IS_FacialExpressionIsLeftWink(eState)
        print IS_FacialExpressionIsRightWink(eState)
        ...
        print FacialExpressionStates[FE_SUPPRISE]
        print FacialExpressionStates[FE_FROWN]
        print FacialExpressionStates[FE_SMILE]
        print FacialExpressionStates[FE_CLENCH]
        '''
        IS_FacialExpressionGetEyeLocation(eState,X,Y)
        print EyeX.value
        print EyeY.value
        IS_FacialExpressionGetEyeLocation(eState,Left,Right)
        print EyeLidLeft.value
        print EyeLidRight.value
    ...
    while (1):
        ...
        if state == 0:
            ...
            if eventType == 64:  # libEDK.IEE_Event_enum.IEE_EmoStateUpdated
                ...
                command = Command()
                command.userInput = "ALERT !!!! BCI FOUND !!!"
                command_msg_publisher.publish(command)
...
command_msg_publisher = rospy.Publisher("smach/command_msg", Command, queue_size=5)

my_service = rospy.Service('/iki_bci', Empty , my_callback) # create the Service
